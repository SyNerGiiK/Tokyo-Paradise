...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedInAnyVehicle(player, false) and vehicleIsOn then
            ...
            -- Vehicle Seatbelt
            if has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~=
                8 then
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        -- Vehicle Seatbelt
        if IsPedInAnyVehicle(player, false) and
            GetIsVehicleEngineRunning(vehicle) then
            if IsControlJustReleased(0, Keys[Config.vehicle.keys.seatbelt]) and
                (has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~=
                    8) then seatbeltIsOn = not seatbeltIsOn end
        end

        -- Vehicle Cruiser
        if IsControlJustPressed(1, Keys[Config.vehicle.keys.cruiser]) and
        ...
        if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and
            ...
            TriggerEvent('trew_hud_ui:setCarSignalLights',
...
AddEventHandler('trew_hud_ui:syncCarLights', function(driver, status)
    ...
    if GetPlayerFromServerId(driver) ~= PlayerId() then
        local driver = GetVehiclePedIsIn(
                           GetPlayerPed(GetPlayerFromServerId(driver)), false)
