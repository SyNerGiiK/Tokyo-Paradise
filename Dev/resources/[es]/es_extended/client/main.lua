...
RegisterNetEvent('secureesx:removeInventoryItem')
AddEventHandler('secureesx:removeInventoryItem', function(item, count)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item.name then
			ESX.PlayerData.inventory[k] = item
			break
...
RegisterNetEvent('secureesx:spawnVehicle')
AddEventHandler('secureesx:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx:tobePed')
AddEventHandler('esx:tobePed', function(model)
	model           = (tonumber(model) ~= nil and tonumber(model) or GetHashKey(model))
