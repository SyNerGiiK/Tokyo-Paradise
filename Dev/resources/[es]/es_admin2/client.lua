...
function getPlayers()
	...
	for _, player in ipairs(GetActivePlayers()) do
		if NetworkIsPlayerActive(player) then
			rpName = nil
			TriggerEvent("drp_esxbridge:client:playerIdentity:get", player, function(name)
				if not name or not name.firstName or not name.lastName then
					rpName = 'Nouveau Joueur'
				else
					rpName = name.firstName .. " " .. name.lastName
				end
			end)
			table.insert(players, {id = GetPlayerServerId(player), name = rpName})
		end
	end
	return players
end